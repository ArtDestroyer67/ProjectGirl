...
public abstract class DeserializationContext extends DatabindContext implements Serializable {
   ...
   public <T> T readValue(JsonParser p, Class<T> type) throws IOException {
      return (T)this.readValue(p, this.getTypeFactory().constructType(type));
   }
   ...
   public <T> T readPropertyValue(JsonParser p, BeanProperty prop, Class<T> type) throws IOException {
      return (T)this.readPropertyValue(p, prop, this.getTypeFactory().constructType(type));
   }

   public <T> T readPropertyValue(JsonParser p, BeanProperty prop, JavaType type) throws IOException {
      JsonDeserializer<Object> deser = this.findContextualValueDeserializer(type, prop);
      return (T)(deser == null ? this.reportBadDefinition(type, String.format("Could not find JsonDeserializer for type %s (via property %s)", type, ClassUtil.nameOf(prop))) : deser.deserialize(p, this));
   }
   ...
   public <T> T reportUnresolvedObjectId(ObjectIdReader oidReader, Object bean) throws JsonMappingException {
      ...
      return (T)this.reportInputMismatch(oidReader.idProperty, msg);
   }
