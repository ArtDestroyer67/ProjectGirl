...
public abstract class BeanDeserializerBase extends StdDeserializer<Object> implements ContextualDeserializer, ResolvableDeserializer, ValueInstantiator.Gettable, Serializable {
   ...
   public void resolve(DeserializationContext ctxt) throws JsonMappingException {
      ...
      for(SettableBeanProperty origProp : this._beanProperties) {
         ...
         SettableBeanProperty var16 = origProp.withValueDeserializer(deser);
         var16 = this._resolveManagedReferenceProperty(ctxt, var16);
         if (!(var16 instanceof ManagedReferenceProperty)) {
            var16 = this._resolvedObjectIdProperty(ctxt, var16);
         }

         NameTransformer xform = this._findPropertyUnwrapper(ctxt, var16);
         if (xform != null) {
            JsonDeserializer<Object> orig = var16.getValueDeserializer();
            JsonDeserializer<Object> unwrapping = orig.unwrappingDeserializer(xform);
            if (unwrapping != orig && unwrapping != null) {
               var16 = var16.withValueDeserializer(unwrapping);
               if (unwrapped == null) {
                  unwrapped = new UnwrappedPropertyHandler();
               }

               unwrapped.addProperty(var16);
               this._beanProperties.remove(var16);
               continue;
            }
         }

         PropertyMetadata md = var16.getMetadata();
         var16 = this._resolveMergeAndNullSettings(ctxt, var16, md);
         var16 = this._resolveInnerClassValuedProperty(ctxt, var16);
         if (var16 != origProp) {
            this._replaceProperty(this._beanProperties, creatorProps, origProp, var16);
         }

         if (var16.hasValueTypeDeserializer()) {
            TypeDeserializer typeDeser = var16.getValueTypeDeserializer();
            if (typeDeser.getTypeInclusion() == JsonTypeInfo.As.EXTERNAL_PROPERTY) {
               if (extTypes == null) {
                  extTypes = ExternalTypeHandler.builder(this._beanType);
               }

               extTypes.addExternal(var16, typeDeser);
               this._beanProperties.remove(var16);
            }
         }
      }
