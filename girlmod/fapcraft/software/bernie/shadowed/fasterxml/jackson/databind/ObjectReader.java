...
public class ObjectReader extends ObjectCodec implements Versioned, Serializable {
   ...
   public <T> T readValue(JsonParser p, Class<T> valueType) throws IOException {
      return (T)this.forType(valueType).readValue(p);
   }

   public <T> T readValue(JsonParser p, TypeReference<?> valueTypeRef) throws IOException {
      return (T)this.forType(valueTypeRef).readValue(p);
   }

   public <T> T readValue(JsonParser p, ResolvedType valueType) throws IOException {
      return (T)this.forType((JavaType)valueType).readValue(p);
   }
   ...
   public <T> T readValue(InputStream src) throws IOException {
      return (T)(this._dataFormatReaders != null ? this._detectBindAndClose(this._dataFormatReaders.findFormat(src), false) : this._bindAndClose(this._considerFilter(this._parserFactory.createParser(src), false)));
   }
   ...
   public <T> T readValue(byte[] src) throws IOException {
      return (T)(this._dataFormatReaders != null ? this._detectBindAndClose(src, 0, src.length) : this._bindAndClose(this._considerFilter(this._parserFactory.createParser(src), false)));
   }

   public <T> T readValue(byte[] src, int offset, int length) throws IOException {
      return (T)(this._dataFormatReaders != null ? this._detectBindAndClose(src, offset, length) : this._bindAndClose(this._considerFilter(this._parserFactory.createParser(src, offset, length), false)));
   }

   public <T> T readValue(File src) throws IOException {
      return (T)(this._dataFormatReaders != null ? this._detectBindAndClose(this._dataFormatReaders.findFormat(this._inputStream(src)), true) : this._bindAndClose(this._considerFilter(this._parserFactory.createParser(src), false)));
   }
   ...
   public <T> MappingIterator<T> readValues(JsonParser p) throws IOException {
      ...
      return this.<T>_newIterator(p, ctxt, this._findRootDeserializer(ctxt), false);
   }
   ...
   public <T> MappingIterator<T> readValues(Reader src) throws IOException {
      ...
      p.nextToken();
      return this.<T>_newIterator(p, ctxt, this._findRootDeserializer(ctxt), true);
   }
   ...
   public final <T> MappingIterator<T> readValues(byte[] src) throws IOException {
      return this.<T>readValues(src, 0, src.length);
   }
   ...
   public <T> MappingIterator<T> readValues(DataInput src) throws IOException {
      ...
      return this.<T>_bindAndReadValues(this._considerFilter(this._parserFactory.createParser(src), true));
   }

   public <T> T treeToValue(TreeNode n, Class<T> valueType) throws JsonProcessingException {
      try {
         return (T)this.readValue(this.treeAsTokens(n), valueType);
   ...
   protected <T> MappingIterator<T> _detectBindAndReadValues(DataFormatReaders.Match match, boolean forceClosing) throws IOException {
      ...
      return match.getReader().<T>_bindAndReadValues(p);
   }
