...
public abstract class StdDeserializer<T> extends JsonDeserializer<T> implements Serializable {
   ...
   protected T _deserializeFromArray(JsonParser p, DeserializationContext ctxt) throws IOException {
      ...
      if (ctxt.hasSomeOfFeatures(F_MASK_ACCEPT_ARRAYS)) {
         t = p.nextToken();
         if (t == JsonToken.END_ARRAY && ctxt.isEnabled(DeserializationFeature.ACCEPT_EMPTY_ARRAY_AS_NULL_OBJECT)) {
            return (T)this.getNullValue(ctxt);
         }

         if (ctxt.isEnabled(DeserializationFeature.UNWRAP_SINGLE_VALUE_ARRAYS)) {
            T parsed = (T)this.deserialize(p, ctxt);
      ...
      T result = (T)ctxt.handleUnexpectedToken(this._valueClass, t, p, (String)null);
      return result;
   }
   ...
   protected T _deserializeWrappedValue(JsonParser p, DeserializationContext ctxt) throws IOException {
      if (p.hasToken(JsonToken.START_ARRAY)) {
         ...
         T result = (T)ctxt.handleUnexpectedToken(this._valueClass, p.getCurrentToken(), p, msg);
         return result;
      } else {
         return (T)this.deserialize(p, ctxt);
      }
   }
