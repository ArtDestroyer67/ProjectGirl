      ...
      if (serClass != null) {
         if (baseType.hasRawClass(serClass)) {
            ...
            try {
               ...
               throw new JsonMappingException((Closeable)null, String.format("Failed to widen type %s with annotation (value %s), from '%s': %s", baseType, serClass.getName(), a.getName(), iae.getMessage()), iae);
            }
         }
      }
               ...
               try {
                  if (keyClass.isAssignableFrom(currRaw)) {
                     ...
                     if (!this._primitiveAndWrapper(currRaw, keyClass)) {
                        throw new JsonMappingException((Closeable)null, String.format("Cannot refine serialization key type %s into %s; types not related", keyType, keyClass.getName()));
                     }

                     keyType = keyType.withStaticTyping();
                  }
               } catch (IllegalArgumentException iae) {
                  throw new JsonMappingException((Closeable)null, String.format("Failed to widen key type of %s with concrete-type annotation (value %s), from '%s': %s", type, keyClass.getName(), a.getName(), iae.getMessage()), iae);
               }
            }
         ...
         if (contentClass != null) {
            if (contentType.hasRawClass(contentClass)) {
               ...
               try {
                  ...
                  throw new JsonMappingException((Closeable)null, String.format("Internal error: failed to refine value type of %s with concrete-type annotation (value %s), from '%s': %s", type, contentClass.getName(), a.getName(), iae.getMessage()), iae);
               }
            }
   ...
   public JavaType refineDeserializationType(MapperConfig<?> config, Annotated a, JavaType baseType) throws JsonMappingException {
      ...
      if (valueClass != null && !baseType.hasRawClass(valueClass) && !this._primitiveAndWrapper(baseType, valueClass)) {
         try {
            ...
            throw new JsonMappingException((Closeable)null, String.format("Failed to narrow type %s with annotation (value %s), from '%s': %s", baseType, valueClass.getName(), a.getName(), iae.getMessage()), iae);
         }
      }
      ...
      if (type.isMapLikeType()) {
         ...
         if (keyClass != null && !this._primitiveAndWrapper(keyType, keyClass)) {
            try {
               ...
               throw new JsonMappingException((Closeable)null, String.format("Failed to narrow key type of %s with concrete-type annotation (value %s), from '%s': %s", type, keyClass.getName(), a.getName(), iae.getMessage()), iae);
            }
         }
      }
      ...
      if (contentType != null) {
         ...
         if (contentClass != null && !this._primitiveAndWrapper(contentType, contentClass)) {
            try {
               ...
               throw new JsonMappingException((Closeable)null, String.format("Failed to narrow value type of %s with concrete-type annotation (value %s), from '%s': %s", type, contentClass.getName(), a.getName(), iae.getMessage()), iae);
            }
         }
      }
