...
public class InstantDeserializer<T extends Temporal> extends JSR310DateTimeDeserializerBase<T> {
   ...
   public T deserialize(JsonParser parser, DeserializationContext context) throws IOException {
      switch (parser.getCurrentTokenId()) {
         case 3:
            return (T)(this._deserializeFromArray(parser, context));
         case 4:
         case 5:
         case 9:
         case 10:
         case 11:
         default:
            return (T)(this._reportWrongToken(parser, context, new JsonToken[]{JsonToken.VALUE_STRING, JsonToken.VALUE_NUMBER_INT, JsonToken.VALUE_NUMBER_FLOAT}));
         case 6:
            ...
            if (string.length() == 0) {
               ...
               if (this._formatter == DateTimeFormatter.ISO_INSTANT || this._formatter == DateTimeFormatter.ISO_OFFSET_DATE_TIME || this._formatter == DateTimeFormatter.ISO_ZONED_DATE_TIME) {
                  ...
                  if (dots >= 0) {
                     try {
                        if (dots == 0) {
                           return (T)this._fromLong(context, Long.parseLong(string));
                        }

                        if (dots == 1) {
                           return (T)this._fromDecimal(context, new BigDecimal(string));
                        }
               ...
               try {
                  ...
                  if (this.shouldAdjustToContextTimezone(context)) {
                     return (T)(this.adjust.apply(value, this.getZone(context)));
                  }
               } catch (DateTimeException e) {
                  value = (T)(this._rethrowDateTimeException(parser, context, e, string));
               }

               return value;
            }
         case 7:
            return (T)this._fromLong(context, parser.getLongValue());
         case 8:
            return (T)this._fromDecimal(context, parser.getDecimalValue());
         case 12:
   ...
   protected T _fromLong(DeserializationContext context, long timestamp) {
      return (T)(context.isEnabled(DeserializationFeature.READ_DATE_TIMESTAMPS_AS_NANOSECONDS) ? (Temporal)this.fromNanoseconds.apply(new FromDecimalArguments(timestamp, 0, this.getZone(context))) : (Temporal)this.fromMilliseconds.apply(new FromIntegerArguments(timestamp, this.getZone(context))));
   }

   protected T _fromDecimal(DeserializationContext context, BigDecimal value) {
      long seconds = value.longValue();
      int nanoseconds = DecimalUtils.extractNanosecondDecimal(value, seconds);
      return (T)(this.fromNanoseconds.apply(new FromDecimalArguments(seconds, nanoseconds, this.getZone(context))));
   }
   ...
   static {
      INSTANT = new InstantDeserializer<Instant>(Instant.class, DateTimeFormatter.ISO_INSTANT, Instant::from, (a) -> Instant.ofEpochMilli(a.value), (a) -> Instant.ofEpochSecond(a.integer, (long)a.fraction), (BiFunction)null, true);
      OFFSET_DATE_TIME = new InstantDeserializer<OffsetDateTime>(OffsetDateTime.class, DateTimeFormatter.ISO_OFFSET_DATE_TIME, OffsetDateTime::from, (a) -> OffsetDateTime.ofInstant(Instant.ofEpochMilli(a.value), a.zoneId), (a) -> OffsetDateTime.ofInstant(Instant.ofEpochSecond(a.integer, (long)a.fraction), a.zoneId), (d, z) -> d.withOffsetSameInstant(z.getRules().getOffset(d.toLocalDateTime())), true);
      ZONED_DATE_TIME = new InstantDeserializer<ZonedDateTime>(ZonedDateTime.class, DateTimeFormatter.ISO_ZONED_DATE_TIME, ZonedDateTime::from, (a) -> ZonedDateTime.ofInstant(Instant.ofEpochMilli(a.value), a.zoneId), (a) -> ZonedDateTime.ofInstant(Instant.ofEpochSecond(a.integer, (long)a.fraction), a.zoneId), ZonedDateTime::withZoneSameInstant, false);
   }
