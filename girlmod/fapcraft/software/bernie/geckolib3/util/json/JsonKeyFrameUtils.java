...
public class JsonKeyFrameUtils {
   private static VectorKeyFrameList<KeyFrame<IValue>> convertJson(List<Map.Entry<String, JsonElement>> element, boolean isRotation, MolangParser parser) throws NumberFormatException, MolangException {
      ...
      for(int i = 0; i < element.size(); ++i) {
         Map.Entry<String, JsonElement> keyframe = (Map.Entry)element.get(i);
         Map.Entry<String, JsonElement> previousKeyFrame = i == 0 ? null : (Map.Entry)element.get(i - 1);
         Double previousKeyFrameLocation = previousKeyFrame == null ? (double)0.0F : Double.parseDouble((String)previousKeyFrame.getKey());
         Double currentKeyFrameLocation = NumberUtils.isCreatable((String)keyframe.getKey()) ? Double.parseDouble((String)keyframe.getKey()) : (double)0.0F;
         ...
         IValue currentXValue = (IValue)(isRotation && xValue instanceof ConstantValue ? ConstantValue.fromDouble(Math.toRadians(-xValue.get())) : xValue);
         IValue currentYValue = (IValue)(isRotation && yValue instanceof ConstantValue ? ConstantValue.fromDouble(Math.toRadians(-yValue.get())) : yValue);
         IValue currentZValue = (IValue)(isRotation && zValue instanceof ConstantValue ? ConstantValue.fromDouble(Math.toRadians(zValue.get())) : zValue);
