...
public abstract class AnimatedGeoModel<T extends IAnimatable> extends GeoModelProvider<T> implements IAnimatableModel<T>, IAnimatableModelProvider<T> {
   ...
   public void setLivingAnimations(T entity, Integer uniqueID, @Nullable AnimationEvent customPredicate) {
      ...
      if (Minecraft.func_71410_x().func_147113_T() && !manager.shouldPlayWhilePaused) {
         ...
         this.seekTime = manager.tick + (double)Minecraft.func_71410_x().func_184121_ak();
      }

      AnimationEvent<T> predicate;
      if (customPredicate == null) {
         predicate = new AnimationEvent<T>(entity, 0.0F, 0.0F, (float)(manager.tick - this.lastGameTickTime), false, Collections.emptyList());
      } else {
   ...
   public double getCurrentTick() {
      return (double)Minecraft.func_71386_F() / (double)50.0F;
   }
}
