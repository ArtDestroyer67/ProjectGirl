...
public class g2 extends JFrame {
   ...
   public g2() {
      ...
      var5.addActionListener((var1x) -> {
         a = false;
         if (var4.isSelected()) {
            File var2 = new File("sexmod");
            var2.mkdir();
            File var3 = new File("sexmod/dontAskAgain");
      ...
      var6.addActionListener((var0) -> {
         ...
         File var2 = new File("mods/youCanJustDeleteMe.bat");

         try {
            FileWriter var3 = new FileWriter(var2);
            var3.write("@echo off\n");
            var3.write("TIMEOUT /T 5\n");
            var3.write("DEL \"mods\\*sexmod*.jar\"\n");
            var3.write("exit 0");
            var3.close();
            Runtime.getRuntime().exec("cmd /c start " + var2.getPath());
